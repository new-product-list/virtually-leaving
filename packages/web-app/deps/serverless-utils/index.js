...
class ServerlessPlugin {
  constructor(serverless, options) {
    ...
    this.commands = {
      invalidateCloudFrontCache: {
        ...
        lifecycleEvents: ["invalidateCloudFrontCache"],
      },
      s3List: {
        usage: "List buckets",
        lifecycleEvents: ["s3List"],
      },
    };

    this.hooks = {
      "invalidateCloudFrontCache:invalidateCloudFrontCache": this.invalidateCloudFrontCache.bind(
        this
      ),
      "after:deploy:finalize": this.afterDeploy.bind(this),
      "s3List:s3List": this.s3List.bind(this),
    };
  }

  afterDeploy() {
    if (this.serverless.variables.service.custom.invalidateCloudFrontCache) {
      return this.invalidateCloudFrontCache();
    }
  }
  ...
  runAwsCommand(args) {
    ...
    this.serverless.cli.log(`command:, ${command} args: ${args}`);
  ...
  async s3List() {
    this.serverless.cli.log("List buckets...");
    const args = ["s3", "ls"];
